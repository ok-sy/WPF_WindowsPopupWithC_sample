...
export type RoleInfoByUserDialogProps = {
  open: boolean;
  onClose: () => void;
...
export default function RoleInfoByUserDialog(props: RoleInfoByUserDialogProps) {
  const { open, onClose, selectedRoleId, onRefresh, teamVersion } = props;
  const api = useApi();
  ...
  const doReload = useCallback(
    async (ctx: ApiRequestContext, roleId: string) => {
      setLoading(true);
      try {
        const { body } = await api.clRoleUser.userListByRole({ ctx, roleId });
        const { userList } = body;
  ...
  useEffect(() => {
    ...
    doReload(ctx, selectedRoleId);
    return () => {
      ctx.canceled = true;
      ctx.cancel?.();
    };
  }, [refreshToken, doReload, selectedRoleId]);
  return (
    <CustomDialog
      open={open}
      ...
      className="RoleInfoByUserDialog-root"
      fullWidth
      maxWidth="xs"
      sx={{}}
    >
