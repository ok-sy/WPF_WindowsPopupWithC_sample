...
export default function ServiceAvgResTimeMonth(props: Props) {
  const { onSubmit, refreshToken } = props;
  const onSubmitFnRef = useRef<Props['onSubmit']>();
  onSubmitFnRef.current = onSubmit;
  const api = useApi();
  const [monthData, setMonthData] = useState<GetDateServiceAvgResponeTimeVo[]>([]);
  ...
  const monthDataApi = useCallback(
    async (ctx: ApiRequestContext) => {
      try {
        const { body } = await api.layout.getMonthServiceAvgResponseTime({ ctx });
        setMonthData(body.list);

        const data = createMonthListChartData(body.list);
        setChartData(data);
        console.log('chartDatachartDatachartDatachartData');
  ...
  const options: any = {
    ...
    decoration: {
      gradient: {
        colors: ['#F3B95F', 'white'], // 그라데이션 색상 배열
        start: 'bottom', // 그라데이션 시작 위치 (top, bottom, left, right)
        end: 'top', // 그라데이션 종료 위치 (top, bottom, left, right)
      },
    },
    // 추가된 부분: onHover 콜백
...
function createMonthListChartData(RATE_RESULT: GetDateServiceAvgResponeTimeVo[]) {
  const labels: string[] = [];
  const cnt: number[] = [];
