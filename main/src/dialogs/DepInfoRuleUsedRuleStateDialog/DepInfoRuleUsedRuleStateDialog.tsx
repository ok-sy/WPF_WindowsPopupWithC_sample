...
import type { ApiRequestContext, RuleDeployHistory, UsedRuleDetailInfo } from '@local/domain';
import { RuleDeployWaitVo } from '@local/domain';
import { useApi } from '@/provider';
import { useCallback, useEffect, useState } from 'react';
import errorCustomHandle from '@/lib/error-custom-handle';

export type DepInfoRuleUsedRuleStateDialogProps = {
  open: boolean;
  onClose: () => void;
  data: RuleDeployHistory;
};

export default function DepInfoRuleUsedRuleStateDialog(props: DepInfoRuleUsedRuleStateDialogProps) {
  const { open, onClose, data } = props;
  const api = useApi();
  const [dataLists, setDataLists] = useState<UsedRuleDetailInfo[]>([]);
  // 목록 조회 API
  const doReload = useCallback(
    async (params: { ruleid: string; ruleVerno: string }, ctx: ApiRequestContext) => {
      try {
        const { body } = await api.rule.usedHistoryRuleList({ ctx, ...params });
  ...
  useEffect(() => {
    ...
    doReload({ ruleid: data?.ruleid, ruleVerno: data?.ruleVerno }, ctx);
  }, [data, doReload]);
  return (
    <CustomDragableDialog
      ...
      fullWidth
      backLightOn
      className="DepInfoRuleUsedRuleStateDialog-root"
      sx={rootSx}
      open={open}
      onClose={onClose}
    >
      <Stack direction="row" justifyContent="space-between" alignItems="center">
        <CustomDialogTitle
          title={`룰에서 사용하는 룰리스트(룰명: ${data?.ruleNm})`}
        ></CustomDialogTitle>
        <Typography sx={{ mr: 3 }}>총{dataLists.length}건</Typography>
      </Stack>
      <DialogContent
        dividers
        className="DepInfoRuleUsedRuleStateDialog-content"
        ...
        <TableContainer
          sx={{
            ...
            '& .MuiTableCell-root': {
              ...
              ':nth-of-type(2)': { width: 150, minWidth: 150, maxWidth: 150 },
              ':nth-of-type(3)': { width: 100, minWidth: 100, maxWidth: 100, textAlign: 'center' },
              ':nth-of-type(4)': { width: 100, minWidth: 100, maxWidth: 100, textAlign: 'center' },
              ':nth-of-type(5)': { width: 150, minWidth: 150, maxWidth: 150, textAlign: 'center' },
              ':nth-of-type(6)': { width: 100, minWidth: 100, maxWidth: 100, textAlign: 'center' },
            },
          }}
        >
