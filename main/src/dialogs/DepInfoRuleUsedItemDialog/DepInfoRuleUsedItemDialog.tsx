...
import type { ApiRequestContext, RuleDeployHistory, UsedItemInfo } from '@local/domain';
import { RuleDeployWaitVo } from '@local/domain';
import { useApi } from '@/provider';
import { useCallback, useEffect, useState } from 'react';
import errorCustomHandle from '@/lib/error-custom-handle';

export type DepInfoRuleUsedItemDialogProps = {
  open: boolean;
  onClose: () => void;
  data: RuleDeployHistory;
};

export default function DepInfoRuleUsedItemDialog(props: DepInfoRuleUsedItemDialogProps) {
  ...
  const doReload = useCallback(
    async (params: { ruleid: string; ruleVerno: string }, ctx: ApiRequestContext) => {
      try {
        const { body } = await api.rule.usedHistoryItemList({ ctx, ...params });
  ...
  return (
    <CustomDragableDialog
      ...
      fullWidth
      backLightOn
      className="DepInfoRuleUsedItemDialog-root"
      sx={rootSx}
      open={open}
      onClose={onClose}
    >
      ...
      <DialogContent
        dividers
        className="DepInfoRuleUsedItemDialog-content"
