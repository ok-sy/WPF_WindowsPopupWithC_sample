...
export default function DelayServiceMaxResTimeMin(props: Props) {
  ...
  const minList = useCallback(
    async (params: { hour?: string }, ctx: ApiRequestContext) => {
      try {
        const { body } = await api.layout.getMinServiceLateTime({ ctx, ...params });
...
function createMinListChartData(RATE_RESULT: GetDateServiceLateTimeVo[]) {
  const labels: string[] = [];
  const cnt: number[] = [];
