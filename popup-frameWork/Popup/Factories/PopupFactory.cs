...
namespace Popup.Factories
{
    public static class PopupFactory
    {
        ...
        public static PopupOptions Create(PopupResponseDto popupDto)
        {
            if (popupDto == null)
            {
                throw new ArgumentNullException(nameof(popupDto));
            }

            FrameworkElement content = popupDto.PopupType
                .Trim()
                .ToUpperInvariant() switch
            {
                ...
                "SURVEY" => CreateSurveyPopupView(popupDto.Content, isQuizMode: false),
                "QUIZ" => CreateSurveyPopupView(popupDto.Content, isQuizMode: true),
                _ => throw new NotSupportedException(
                    $"지원하지 않는 팝업 종류입니다: {popupDto.PopupType}")
            };
        ...
        private static TextPopupView CreateTextPopupView(JsonElement contentJson)
        {
            TextPopupContentDto contentDto =
                contentJson.Deserialize<TextPopupContentDto>(JsonOptions)
         ...
         * 제목/설명/이미지 전용 크기 옵션은 더 이상 View 생성에 사용하지 않는다.
         */
        private static ImagePopupView CreateImagePopupView(JsonElement contentJson)
        {
            ImagePopupContentDto contentDto =
                contentJson.Deserialize<ImagePopupContentDto>(JsonOptions)
                ?? throw new InvalidOperationException(
        ...
        private static VideoPopupView CreateVideoPopupView(JsonElement contentJson)
        {
            VideoPopupContentDto contentDto =
                contentJson.Deserialize<VideoPopupContentDto>(JsonOptions)
                ?? throw new InvalidOperationException(
                    "VIDEO 팝업 content 변환에 실패했습니다.");

            return new VideoPopupView(
                videoTitle: contentDto.VideoTitle,
                videoPath: contentDto.VideoUrl,
                videoDescription: contentDto.Description,
                showDescription: contentDto.ShowDescription);
        }
            ...
            bool isQuizMode)
        {
            SurveyPopupContentDto contentDto =
                contentJson.Deserialize<SurveyPopupContentDto>(JsonOptions)
                ?? throw new InvalidOperationException(
            ...
            return new SurveyPopupView(
                title: contentDto.SurveyTitle,
                description: contentDto.Description,
                questions: questions,
                isQuizMode: isQuizMode,
                passingScore: contentDto.PassingScore);
        }
